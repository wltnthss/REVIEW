...
### FK 제약이 걸려있지 않을 때 인덱스 미생성으로 인한 성능저하

```SQL
-- FK 인덱스 미생성으로 FULL TABLE SCAN 발생 -> 성능저하
SELECT COUNT(B.학번)
FROM 학사기준 A, 수강신청 B
...
* 데이터 모델 관계에 의해 상속받은 FK속성들은 WHERE절에서 조인으로 많이 이용되므로 FK 인덱스를 생성해야 성능이 좋은 경우가 빈번하다.
* 학사기준에 데이터 5만건, 수강신청에 데이터 500만 건이 있다 가정할 때, 수강신청 테이블에 있는 학사기준번호가 WHERE절에 비교자로 들어오지는 않았지만, 수강신청 테이블에서 상속받은 학사기준번호에대해 인덱스를 생성하지 않았으므로 500만 건의 수강신청 테이블이 FULL TABLE SCAN 이 발생한다.

### 분산 데이터베이스와 성능

* 분산 데이터베이스 : 여러 곳으로 분산되어있는 데이터베이스를 하나의 가상 시스템으로 사용할 수 있도록 한 데이터베이스
* ```데이터를 분산 환경으로 구성하였을 때 통합된 데이터베이스에서는 제공할 수없는 빠른 성능을 제공```한다.
* 트랜잭션 집중 및 네트워크 부하에 따른 성능 저하에 대한 문제점이 해결 가능하다.

**분산 데이터베이스의 적용 기법(프로젝트에서 가장 중요하게 이용되는 기법)**

1. 테이블 위치 분산

* 테이블 구조는 변하지 않으며, 테이블의 위치를 각각 다르게 위치시킨다.
* 예를 들어 자재품목은 본사에서 관리하고, 각 지사별로 자재품목을 이용하여 재품을 생산하는 경우, 본사와 지사 단위로 분산시킨다.

2. 테이블 분할 분산

* 위치만 다르게 두는 것뿐만 아니라 각각의 테이블을 쪼개어 분산하는 방법이다.
* 로우 단위로 분할하는 수평분할과 칼럼 단위로 분할하는 수직분할 두 가지 방법에 따라 나뉜다.

3. 테이블 복제 분산

* 동일한 테이블을 다른 지역이나 서버에서 동시에 생성하여 관리하는 유형이다.
* 마스터 DB에서 테이블의 일부 내용만 위치시키는 부분복제, 마스터DB 테이블 내용을 서버에 존재시키는 광역복제가 있다.

**부분복제**

* 본사와 지사간에 데이터의 중복이 항상 발생하며 지사에 데이터가 먼저 발생하고 본사에 데이터는 지사에 데이터를 통합하여 발생한다.
* 다른 지역간의 데이터를 복제하는데 시간과 부하 발생으로 실시간 처리보다는 야간 배치 작업에 의해 수행된다.

**광역복제**

* 통합된 테이블을 본사에 가지고 있으며 각 지사에도 본사와 동일한 데이터를 모두 가지고 있는 형태이다.
